...
`reqwest` 关掉默认 feature 改用 `rustls-tls`，避免链接系统 OpenSSL，同样为了可移植。
...
mod client_tests {
    ...
    #[test]
    fn 不足一百只不分片() {
        assert_eq!(chunk_symbols(&syms(30)).len(), 1);
    }

    #[test]
    fn 正好一百只不分片() {
        assert_eq!(chunk_symbols(&syms(100)).len(), 1);
    }

    #[test]
    fn 超过一百只按百分片() {
        // 腾讯实测上限 100 只/请求，请求 300 只只静默返回前 100
        let chunks = chunk_symbols(&syms(250));
...
mod app_tests {
    ...
    fn press(code: KeyCode) -> KeyEvent {
        KeyEvent { code, modifiers: KeyModifiers::NONE, kind: KeyEventKind::Press, state: Default::default() }
    }
