...
2. **行情有真实延迟，且各市场不同** —— A股实时，港股慢 15 分钟，日股慢约 20 分钟。UI 必须逐条标注真实延迟，不能假装都是实时。
3. **历史 K 线深度严重不足** —— 腾讯上限 640 根，且对美股/日股基本不返回。而 `EMA576` 需要约 **1330 根**日线才能收敛到 1% 误差以内。历史数据必须自行落库积累。
